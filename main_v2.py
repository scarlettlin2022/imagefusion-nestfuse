...
def main():
    ...
    train_path=[FDG_train,MRI_train]
    valid_path=[FDG_valid,MRI_valid]
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        for i in range(1,2):
            weights_pixel_ssim=args.ssim_weight[i]
            train(weights_pixel_ssim,train_dataloader, optimizer, nest_model, epoch)
...
def train(weights_pixel_ssim,train_dataloader, optimizer, model, epoch):
    ...
    #-----------model
        ...
        ssim_loss_value = 0.
        pixel_loss_value = 0.
        pixel_loss_FDG = mse_loss(outputs, FDG_1c)*int(opt.pixelRatio) / len(outputs_cpu)
        ...
        ssim_loss_MRI = (1-ssim_loss(outputs, MRI_1c, normalize=True))*int(opt.ssimRatio) / len(outputs_cpu)
        ...
        # total loss
        total_loss = pixel_loss_value + weights_pixel_ssim * ssim_loss_value
        ...
        if (iteration + 1) % args.log_interval == 0:
            ...
            print(mesg)'''
            print("Epoch {}:\t[{}/{}]\t pixel loss FDG: {:.6f}, pixel loss MRI: {:.6f}, ssim loss FDG: {:.6f}, ssim loss MRI: {:.6f},  pixel: {:.6f}, ssim: {:.6f}, total: {:.6f}\n".format(epoch, iteration, len(training_data_loader), pixel_loss_FDG, pixel_loss_MRI, ssim_loss_FDG, ssim_loss_MRI, pixel_loss_value.item(), ssim_loss_value.item(),  total_loss.item()))
