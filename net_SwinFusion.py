#originally from SwinFusion/models/network_swinfusion1.py
...
class SwinFusion(nn.Module):
    ...
    def forward_features_Ex_A(self, x):
        ...
        for layer in self.layers_Ex_A:
            x_size = (int(pow(x.shape[1],1/2)), int(pow(x.shape[1],1/2)))
            #print(x_size)
            x = layer(x, x_size)
            #print(x.shape)
    ...
    def forward(self, A, B):
        ...
        #print("self.forward_features_Ex_A",x.shape)
        #print(torch.mean(x))
        y = self.forward_features_Ex_B(y)
        #print("self.forward_features_Ex_B",y.shape)
        #print(torch.mean(y))
        x = self.forward_features_Fusion(x, y)
        #print("self.forward_features_Fusion",x.shape)
        #print(torch.mean(x))
        x = self.forward_features_Re(x)                  
        #print("self.forward_features_Re",x.shape)
        #print(torch.mean(x))
...
if __name__ == "__main__":
    ...
    net = SwinFusion(img_size=img_size, patch_size=patch_size).cuda()
    x = torch.randn((1, 1, img_size, img_size)).cuda()
    y = torch.randn((1, 1, img_size, img_size)).cuda()
    outputs = net(x,y)
    print(outputs.shape)