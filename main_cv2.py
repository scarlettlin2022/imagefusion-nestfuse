    ...
    for fold, (train_ids, valid_ids) in enumerate(kfold.split(dataset)):
        ...
        for epoch in range(opt.start_epoch, opt.nEpochs + 1):
            for i in range(2,3):
                ...
                train(ratio_ssim,trainloader, optimizer, nest_model, epoch, fold, train_df)
                valid(ratio_ssim,validloader, save_best_loss_model, nest_model, epoch, fold, valid_df) 

                if epoch%20==0:
                    save_checkpoint(nest_model, epoch, ratio_ssim, fold)
                    x=range(1,epoch+1)
