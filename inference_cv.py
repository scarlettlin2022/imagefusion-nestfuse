...
parser.add_argument("--dtstCh1", default='src/FDG_test', type=str, help="root folder: channel 1 of input dataset")
parser.add_argument("--dtstCh2", default='src/MRI_test', type=str, help="root folder: channel 2 of input dataset")
...
def main():
    ...
    inference(dataset,testloader, nest_model, opt.start_epoch)
...
def inference(dataset,dataloader, model, epoch):
    ...
    for iteration, data in enumerate(dataloader, 1):
        ...
        if opt.fusion==0:
            f = model.fusion_channel_spatial(en_FDG, en_MRI, f_type)
        elif opt.fusion==1:
            f = model.fusion_linear_attention(en_FDG, en_MRI, opt.heads)
        elif opt.fusion==2:
            f = model.fusion_hydra_attention(en_FDG, en_MRI, opt.heads)
        elif opt.fusion==3:
            f = model.fusion_efficient_attention(en_FDG, en_MRI, opt.heads)
        # decoder
    ...
    #-----------model
        ...
        for n in range(len(outputs_cpu)):
            ...
            fused_filename=batch_path[n][28:-4]+".tif"#str(X)+"_fused.tif"
            source1_filename=batch_path[n][28:-4]+".tif"
            source2_filename=batch_path[n][28:-4]+".tif"
