#originally from Swin-Unet/networks/swin_unet.py 
...
class SwinUNet(nn.Module):
    ...
    def __init__(self, img_size=112, patch_size=2, in_chans=1, num_classes=1,
                 ...
                 use_checkpoint=False, final_upsample="expand_first", up_x4=True,**kwargs):
        super().__init__()
    ...
    def feature_fusion(self,x_downsample, y_downsample):
        ...
        for i in range(len(self.layers_fusion)):
            ...
            #print(i, ": xy_cross", xy_cross.shape)
    ...
    def forward(self, x, y):
        ...
        y, y_downsample = self.forward_features(y)
        xy_downsample = self.feature_fusion(x_downsample, y_downsample)
        xy = self.forward_up_features(xy_downsample[-1],xy_downsample)
        xy = self.up_x4(xy)
...
if __name__ == "__main__":
    x = torch.randn(24,1,112,112)
    x=x.cuda()
    y = torch.randn(24,1,112,112)
    y=y.cuda()
    print("input",x.shape)
    ...
    print(swinunet(x,y).shape)
    end_time = time.time()

    elapsed_time = end_time - start_time
    print("Elapsed time: ", elapsed_time) 
    print(swinunet.flops())