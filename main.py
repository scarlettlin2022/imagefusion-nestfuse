...
def main():
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        for i in range(1,2):
            train(i,train_dataloader, optimizer, nest_model, epoch)
...
def train(i,train_dataloader, optimizer, model, epoch):
    ...
    #-----------model
        ...
        ssim_loss_value = 0.
        pixel_loss_value = 0.
        pixel_loss_FDG = mse_loss(outputs, FDG_1c)
        pixel_loss_MRI = mse_loss(outputs, MRI_1c)
        ssim_loss_FDG = ssim_loss(outputs_cpu, FDG_1c_cpu, normalize=True)
        ssim_loss_MRI = ssim_loss(outputs_cpu, MRI_1c_cpu, normalize=True)
        
        pixel_loss_value += pixel_loss_FDG
        pixel_loss_value += pixel_loss_MRI

        ssim_loss_value += (1-ssim_loss_FDG)
        ssim_loss_value += (1-ssim_loss_MRI)
            
        ssim_loss_value /= len(outputs_cpu)
        pixel_loss_value /= len(outputs_cpu)
        ...
        if (iteration + 1) % args.log_interval == 0:
            mesg = "{}\t SSIM weight {}\tEpoch {}:\t[{}/{}]\t pixel loss: {:.6f}\t ssim loss: {:.6f}\t total: {:.6f}".format(
                time.ctime(), i, epoch + 1, iteration, len(training_data_loader),
            ...
            print(mesg)
            print("pixel loss FDG: {:.6f}, pixel loss MRI: {:.6f}, ssim loss FDG: {:.6f}, ssim loss MRI: {:.6f}\n".format(pixel_loss_FDG, pixel_loss_MRI, ssim_loss_FDG, ssim_loss_MRI))
